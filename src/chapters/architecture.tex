...
serialised forests from disk and runs inference in under a second. Everything
...
Data collection runs inside a three-container Docker Compose setup.
Figure~\ref{fig:topology} shows the layout.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{src/img/arch/fig_topology.pdf}
  \caption{Docker Compose test topology. The network simulator sits between
           client and server and applies \texttt{tc netem} impairments on
...
172.28.0.2 toward the client, 172.28.100.2 toward the server. Static routes
are added at container startup so all traffic between client and server goes
through the simulator. Neither side can reach the other directly.
...
Bandwidth is shaped with \texttt{tc tbf} in series with netem. One run fixes
...
There are two deployment modes. In cooperative mode, a lightweight
...
In passive mode, no server-side process is needed. The prober estimates
